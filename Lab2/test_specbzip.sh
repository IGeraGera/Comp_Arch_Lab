#!/bin/bash
#This is a script for automating the benchmark testing process, while making the conf_script_bzip.ini file for reading results with read_results.sh
...
#Adding the first line of the conf_script_bzip.ini
echo "[Benchmarks]" >> conf_script_bzip.ini
# Adding names of tests in conf_script_bzip.ini and running tests
for dcache in {32,64,128}; do
	((iter+=1))
	filename="speczip_dcache_"$dcache""
	echo $filename >> conf_script_bzip.ini
...
mv conf_script_bzip.ini specbzip_results/

inipath="specbzip_results/conf_script_bzip.ini"
