...
def getEDAP(mcpatoutputFile, statsFile):
    leakage, dynamic, area, peak_power = readMcPAT(mcpatoutputFile)
...
def readMcPAT(mcpatoutputFile):
    ...
    return (float(leakage), float(dynamic), float(area), float(peak_power))
